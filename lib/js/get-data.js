...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      var weight = byCodes('3141-9').concat(byCodes('29463-7'));
      ...
      weight = weight.sort((a, b) => (a.effectiveDateTime < b.effectiveDateTime) ? 1 : -1);
      height = height.sort((a, b) => (a.effectiveDateTime < b.effectiveDateTime) ? 1 : -1);
      ...
      final_weight_id = weight[0].id;

    });
    // dd865541-4cd9-4ed7-9b3e-a7ae2ccc5e7b
  ...
  client.request("/MedicationRequest?patient=" + client.patient.id, {
    ...
    meds.entry.forEach(d => {
      let name = d.resource.medicationCodeableConcept.coding.filter(i => {
        i.userSelected
      })[0].display;
      console.log(name)
      console.log(d.resource.medicationCodeableConcept.coding.filter(i => {
        i.userSelected
      }))
      medResults.push(name);
    })
  ...
  function addWeightAnnotation() {
    ...
    var retnHtml = 'XW - '+Date().toISOString("YYYY-MM-DDThh:mm:ss+zz:zz")+' - '+annotation;
    client.update({
      type: "Observation",
      id: final_weight_id,
      resource: {
